...
import ServiceLogGuideModal from '@/components/service/ServiceLogGuideModal'
...
function clientLabel(log: Partial<ServiceLog> | null | undefined) {
  return log?.client?.name || log?.client_name || '(거래처 없음)'
}
...
export default function ServiceLogWorkspace({ logKind = 'service' }: { logKind?: ServiceLogKind }) {
  const meta = KIND_META[logKind] || KIND_META.service
  ...
  const fetchLogs = useCallback(async (opts?: { clearPending?: boolean }) => {
    setLoading(true)
    const result = await getServiceLogsAction(logKind)
    if (result.success) {
    ...
    if ((result as any).message) {
      setSchemaWarning(String((result as any).message))
      setLogs([])
    }
    setLoading(false)
  ...
  const displayLogs = useMemo(() => {
    return logs.map((log) => {
      ...
      if ('inventory_id' in patch) {
        ...
        const opts = (cid ? machineCache.current[cid] || machineOptions[cid] : null) || []
        const opt = opts.find((o: { value: string; label: string }) => o.value === patch.inventory_id)
  ...
  const sortedLogs = useMemo(() => {
    ...
    list.sort((a, b) => {
      const nameA = a.client?.name || a.client_name || ''
      const nameB = b.client?.name || b.client_name || ''
  ...
  const displayRowGroups = useMemo(() => {
    ...
    if (sortBy === 'status') {
      ...
      groups = [...groups].sort((a, b) => {
        ...
        const nameA = a.primary.client?.name || a.primary.client_name || ''
        const nameB = b.primary.client?.name || b.primary.client_name || ''
    ...
    try {
      for (const [id, fields] of Object.entries(pending)) {
        ...
        if (isDummyId(id)) {
          ...
          const result = await createServiceLogAction(
            {
              ...
              client_name: merged.client_name || merged.client?.name || null,
          ...
          if (!result.success) {
            errors.push(`등록 실패 (${clientLabel(merged)}): ${result.message}`)
            continue
          }
          savedIds.push(id)
        } else {
  ...
  const renderLogRow = (
    ...
    const cid = log.client_id || ''
    const machineOpts = (cid ? machineOptions[cid] || machineCache.current[cid] : null) || [
      { value: '', label: '(기기 없음)' },
    ...
    return (
      ...
      >
        <td
          ...
          title="더블클릭: 일지 수정 · 거래처명 클릭으로도 수정"
        >
        ...
        <td
          className={styles.td}
          title={locked ? undefined : '클릭: 일지 수정'}
        >
          <span className={styles.clientNameCell}>
            ...
            {canExpand ? (
              <button
                ...
                title={opts.expanded ? '과거 방문 접기' : '과거 방문 펼치기'}
                aria-label={opts.expanded ? '과거 방문 접기' : '과거 방문 펼치기'}
                onClick={(e) => {
                  e.stopPropagation()
                  toggleExpand(opts.groupKey)
                }}
              >
            ...
            <button
              type="button"
              className={`${styles.clientNameBtn} ${log.client?.is_deleted ? styles.clientDeletedName : ''}`}
              title={locked ? undefined : '클릭: 일지 수정'}
              disabled={locked}
              onClick={(e) => {
                e.stopPropagation()
        ...
        <EditableCell
          ...
          disabled={locked}
          onBeforeEdit={() => {
            if (log.client_id) ensureMachines(log.client_id)
          }}
          display={
            log.inventory
  ...
  return (
    <div className={styles.container}>
      ...
      {galleryLog && (
        <ServiceImageGallery
          logId={galleryLog.id}
          clientName={clientLabel(galleryLog)}
          locked={locked}
          onClose={() => {
            setGalleryLog(null)
            fetchLogs()
      ...
      <ServiceLogGuideModal isOpen={guideOpen} onClose={() => setGuideOpen(false)} />
    </div>
  )
}
